...
describe('ChatMessage e2e test', () => {
  ...
  it('should create and save ChatMessages', async () => {
    ...
    const selectedIsReceived = chatMessageUpdatePage.getIsReceivedInput();
    if (await selectedIsReceived.isSelected()) {
      await chatMessageUpdatePage.getIsReceivedInput().click();
      expect(await chatMessageUpdatePage.getIsReceivedInput().isSelected(), 'Expected isReceived not to be selected').to.be.false;
    } else {
      await chatMessageUpdatePage.getIsReceivedInput().click();
      expect(await chatMessageUpdatePage.getIsReceivedInput().isSelected(), 'Expected isReceived to be selected').to.be.true;
    }
