...
public class ChatMessageResourceIT {
    ...
    public void createChatMessage() throws Exception {
        ...
        assertThat(testChatMessage.isIsRecieved()).isEqualTo(DEFAULT_IS_RECIEVED);
    ...
    @Test
    @Transactional
    public void getAllChatMessagesByIsRecievedIsEqualToSomething() throws Exception {
        // Initialize the database
        chatMessageRepository.saveAndFlush(chatMessage);

        // Get all the chatMessageList where isRecieved equals to DEFAULT_IS_RECIEVED
        defaultChatMessageShouldBeFound("isRecieved.equals=" + DEFAULT_IS_RECIEVED);

        // Get all the chatMessageList where isRecieved equals to UPDATED_IS_RECIEVED
        defaultChatMessageShouldNotBeFound("isRecieved.equals=" + UPDATED_IS_RECIEVED);
    }

    @Test
    @Transactional
    public void getAllChatMessagesByIsRecievedIsInShouldWork() throws Exception {
        // Initialize the database
        chatMessageRepository.saveAndFlush(chatMessage);

        // Get all the chatMessageList where isRecieved in DEFAULT_IS_RECIEVED or UPDATED_IS_RECIEVED
        defaultChatMessageShouldBeFound("isRecieved.in=" + DEFAULT_IS_RECIEVED + "," + UPDATED_IS_RECIEVED);

        // Get all the chatMessageList where isRecieved equals to UPDATED_IS_RECIEVED
        defaultChatMessageShouldNotBeFound("isRecieved.in=" + UPDATED_IS_RECIEVED);
    }

    @Test
    @Transactional
    public void getAllChatMessagesByIsRecievedIsNullOrNotNull() throws Exception {
        // Initialize the database
        chatMessageRepository.saveAndFlush(chatMessage);

        // Get all the chatMessageList where isRecieved is not null
        defaultChatMessageShouldBeFound("isRecieved.specified=true");

        // Get all the chatMessageList where isRecieved is null
        defaultChatMessageShouldNotBeFound("isRecieved.specified=false");
    }
    ...
    @Test
    @Transactional
    public void getAllChatMessagesByOffensiveMessageIsEqualToSomething() throws Exception {
        // Initialize the database
        OffensiveMessage offensiveMessage = OffensiveMessageResourceIT.createEntity(em);
        em.persist(offensiveMessage);
        em.flush();
        chatMessage.addOffensiveMessage(offensiveMessage);
        chatMessageRepository.saveAndFlush(chatMessage);
        Long offensiveMessageId = offensiveMessage.getId();

        // Get all the chatMessageList where offensiveMessage equals to offensiveMessageId
        defaultChatMessageShouldBeFound("offensiveMessageId.equals=" + offensiveMessageId);

        // Get all the chatMessageList where offensiveMessage equals to offensiveMessageId + 1
        defaultChatMessageShouldNotBeFound("offensiveMessageId.equals=" + (offensiveMessageId + 1));
    }
    ...
    public void updateChatMessage() throws Exception {
        ...
        assertThat(testChatMessage.isIsRecieved()).isEqualTo(UPDATED_IS_RECIEVED);
