...
public class ChatUserResourceIT {
    ...
    public void createChatUser() throws Exception {
        ...
        assertThat(testChatUser.getImage()).isEqualTo(DEFAULT_IMAGE);
        assertThat(testChatUser.getImageContentType()).isEqualTo(DEFAULT_IMAGE_CONTENT_TYPE);
    }
    ...
    @Test
    @Transactional
    public void getAllChatUsersByOffensiveMessageIsEqualToSomething() throws Exception {
        // Initialize the database
        OffensiveMessage offensiveMessage = OffensiveMessageResourceIT.createEntity(em);
        em.persist(offensiveMessage);
        em.flush();
        chatUser.addOffensiveMessage(offensiveMessage);
        chatUserRepository.saveAndFlush(chatUser);
        Long offensiveMessageId = offensiveMessage.getId();

        // Get all the chatUserList where offensiveMessage equals to offensiveMessageId
        defaultChatUserShouldBeFound("offensiveMessageId.equals=" + offensiveMessageId);

        // Get all the chatUserList where offensiveMessage equals to offensiveMessageId + 1
        defaultChatUserShouldNotBeFound("offensiveMessageId.equals=" + (offensiveMessageId + 1));
    }
    ...
    public void updateChatUser() throws Exception {
        ...
        assertThat(testChatUser.getImage()).isEqualTo(UPDATED_IMAGE);
        assertThat(testChatUser.getImageContentType()).isEqualTo(UPDATED_IMAGE_CONTENT_TYPE);
    }
