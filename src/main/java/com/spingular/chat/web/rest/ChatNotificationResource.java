...
public class ChatNotificationResource {
    ...
    public ResponseEntity<List<ChatNotificationDTO>> getAllChatNotifications(ChatNotificationCriteria criteria) {
        log.debug("REST request to get ChatNotifications by criteria: {}", criteria);
        List<ChatNotificationDTO> entityList = chatNotificationQueryService.findByCriteria(criteria);
